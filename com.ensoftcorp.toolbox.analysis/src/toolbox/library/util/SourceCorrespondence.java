...
public class SourceCorrespondence implements Comparable<SourceCorrespondence> {
	private com.ensoftcorp.atlas.java.core.index.common.SourceCorrespondence sc;
	...
	private SourceCorrespondence(
			com.ensoftcorp.atlas.java.core.index.common.SourceCorrespondence sc) {
		this.sc = sc;
	}
	...
	public static Collection<SourceCorrespondence> getSourceCorrespondents(Q q) {
		...
		for (GraphElement node : nodes) {
			Object name = node.attr().get(Attr.Node.NAME);
			...
			if (scList != null) {
				for (com.ensoftcorp.atlas.java.core.index.common.SourceCorrespondence scListItem : (List<com.ensoftcorp.atlas.java.core.index.common.SourceCorrespondence>) scList) {
					if (name != null) {
						sourceCorrespondents
								.add(new SourceCorrespondence(
										(com.ensoftcorp.atlas.java.core.index.common.SourceCorrespondence) scListItem,
										name.toString()));
					} else {
						sourceCorrespondents
								.add(new SourceCorrespondence(
	...
	public static SourceCorrespondence getSourceCorrespondent(GraphElement ge) {
		...
		Object name = ge.attr().get(Attr.Node.NAME);
	...
	private static String summarize(
			Collection<SourceCorrespondence> allSources,
			Collection<SourceCorrespondence> methods) {
		...
		// correspondents
		// note not using a set here, because the Q should insure there are no
		// duplicates
		// also duplicate method names may be present in a given file
		...
		for (Entry<String, SortedSet<LineNumber>> entry : filesToLineNumbers
			...
			if (filesToMethods.containsKey(entry.getKey())) {
				LinkedList<String> methodNames = filesToMethods.get(entry
						.getKey());
